...
Write-Host "âœ“ Server initialized and returned tools list" -ForegroundColor Green
Write-Host ""
...
if ($result2 -match '"isError":\s*true') {
  Write-Host "âœ— Scrape failed - check output below" -ForegroundColor Red
  Write-Host $result2
} else {
  Write-Host "âœ“ Successfully scraped example.com" -ForegroundColor Green
...
if ($result3 -match '"isError":\s*true') {
  Write-Host "âœ— Search failed - check output below" -ForegroundColor Red
  Write-Host $result3
} else {
  Write-Host "âœ“ Successfully searched web" -ForegroundColor Green
  # Count results
...
if ($result4 -match '"isError":\s*true') {
  Write-Host "âœ— Answers failed - check output below" -ForegroundColor Red
  Write-Host $result4
} else {
  Write-Host "âœ“ Successfully got AI answer" -ForegroundColor Green
